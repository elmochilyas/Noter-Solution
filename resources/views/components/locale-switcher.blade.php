...
<a href="{{ $path }}"
   class="inline-flex items-center gap-1.5 rounded-md border border-stone-200 px-3 py-1.5 text-sm font-medium text-stone-600 hover:bg-stone-100 hover:text-ink transition-fast"
   aria-label="{{ $other === 'ar' ? 'العربية' : 'Français' }}">
    <span>{{ $other === 'ar' ? 'العربية' : 'FR' }}</span>
